...
class ConfigurationManager {
  constructor() {
    ...
    this.config = {
      ...
      modulesWidth: [1000, 1000, 1000, 1000], // Largeurs des modules
  ...
  validate(path, value) {
    ...
    if (path === 'width' || path === 'height') {
      ...
      if (numValue < constraint.min) {
        return `La ${path === 'width' ? 'largeur' : 'hauteur'} doit être d'au moins ${constraint.min}mm`;
      }
      if (numValue > constraint.max) {
        return `La ${path === 'width' ? 'largeur' : 'hauteur'} ne peut pas dépasser ${constraint.max}mm`;
      }
    }
    ...
    if (path.startsWith('modulesWidth')) {
      ...
      const availableWidth = this.get('width') - (this.get('modulesCount') + 1) * 40; // 40mm par montant
      
      if (Math.abs(totalWidth - availableWidth) > 10) { // Tolérance de 10mm
