// Fonction utilitaire pour synchroniser les modules depuis le DOM
function syncModulesFromDOM(config) {
  const modulesCount = parseInt(config.modulesCount) || 1;
  ...
  while (config.modules.length < modulesCount) {
    config.modules.push({ width: 800 }); // valeur par défaut
  }
  while (config.modules.length > modulesCount) {
    config.modules.pop();
  }
  ...
  for (let i = 0; i < modulesCount; i++) {
    ...
    if (input && input.value) {
      const newWidth = parseInt(input.value) || config.width / modulesCount;
      config.modules[i].width = newWidth;
      console.log(`📝 Module ${i + 1} synchronisé: ${newWidth}mm`);
    } else {
      // Si pas d'input, utiliser largeur par défaut
      const defaultWidth = (config.width || 4000) / modulesCount;
      config.modules[i].width = defaultWidth;
      console.log(`📝 Module ${i + 1} défaut: ${defaultWidth}mm`);
    }
  }
